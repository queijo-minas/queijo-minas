...
// Seleciona o elemento do header diretamente
const scrollElement = document.querySelector('header');


// Função que altera a cor do fundo do header com base na rolagem da página
window.addEventListener('scroll', () => {
  const scrollPosition = window.scrollY; // Pega a posição atual do scroll da página
  // Se a posição do scroll for menor que 200 pixels, torna o fundo transparente
  if (scrollPosition < 200) {
    scrollElement.style.backgroundColor = '#00000000'; // Transparente no topo da página
  } else {
    // Se a rolagem passar de 200 pixels, muda o fundo para uma cor sólida
    scrollElement.style.backgroundColor = '#002147'; // Fundo azul escuro após rolar
  }
});
